// src/pages/Defect_Management_Upstream_INT_POPUP 

import '../MyApp.css'
import './Defect_Management_Upstream_INT_POPUP.css'
...
export interface UPSTREAM_PopupProps {
  isOpen: boolean
  onClose: () => void
...
export default function Defect_Management_Upstream_INT_POPUP({
  isOpen,
  onClose,
  ...
  const [scrapReturn, setScrapReturn] = useState<string>('') // 문자열
  ...
  const [machinesOptions, setMachinesOptions] = useState<string[]>([])
  ...
  const loadAgg = async () => {
    ...
    try {
      ...
      // 3) machine 옵션 (Work Center 선택 시에만)
      let machineCodes: string[] = []
      if (workCenter) {
        const machinesResp = await fetchMachines({
          // TODO:: Plant API 생성 후 선택 값으로 Plant 넣기
          plant,
          work_center: workCenter,
          // material_code 연쇄 필터는 현재 사용하지 않음
        })
        machineCodes = machinesResp.data.map((m) => m.machine_cd)
      }
      setMachinesOptions(machineCodes)
      if (machine && !machineCodes.includes(machine)) {
        setMachine('')
      }

      // 3) Material 옵션 (remain > 0, Work Center + Line + machine 선택 시에만)
      ...
      if (scrapReturn === 'R' && workCenter && line && material) {
  ...
  }, [plant, workCenter, line, machine, material, scrapReturn, isOpen])

  // 현재 응답에서 허용되는 옵션 목록(서버가 필터 반영해 줌)
  const lines = linesOptions
  const machines = machinesOptions
  ...
  useEffect(() => {
    // console.log('gradeTypeOptions state:', gradeTypeOptions.length)
  }, [gradeTypeOptions])
  ...
  const onSubmit = async () => {
    ...
    console.log('[Defect_UPSTREAM_POPUP] Saved payload:', payload)

    // 최소 검증
    const defectQty = Number(qty)
    if (!plant || !workCenter || !line || !machine || !material) {
      console.error('[Defect_UPSTREAM_POPUP] plant/workCenter/line/machine/material is required')
      return
    }
    // qty 필수
    if (!qty || Number.isNaN(defectQty)) {
      console.error('[Defect_UPSTREAM_POPUP] qty must be a valid number')
      return
    }
    // Return일 때 component 필수
    if (scrapReturn === 'R' && !componentVal) {
      console.error('[Defect_UPSTREAM_POPUP] component is required when Return is selected')
      return
    }
    // gradeType 필수
    if (!gradeType) {
      console.error('[Defect_UPSTREAM_POPUP] gradeType is required')
      return
    }
    // reason 필수
    if (!reason) {
      console.error('[Defect_UPSTREAM_POPUP] reason is required')
      return
    }
    // division 필수
    if (!side) {
      console.error('[Defect_UPSTREAM_POPUP] division(side) is required')
      return
    }
    //inspectionType 필수

    const request: DefectDistributeRequest = {
      plant: plant, // TODO:: Plant API 생성 후 선택 값으로 Plant 넣기
      ...
      defect_form: 'UPSTREAM',
      log: {
        division: side || undefined,
        defect_source: gradeType || undefined,
        defect_decision: scrapReturn || undefined,
    ...
    try {
      ...
      if ('code' in result && result.code === 'DEFECT_QTY_EXCEEDED') {
        ...
        console.log('[Defect_UPSTREAM_POPUP] Defect qty exceeded')
        ...
        // 200 정상 성공 케이스
        console.log('[Defect_UPSTREAM_POPUP] Distribute defects result:', result)
        // TODO: 성공 메시지/리셋 처리
      }
      ...
      console.error('[Defect_UPSTREAM_POPUP] Distribute defects error:', msg)
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
          ...
          >
            <span>Upstream Internal Defect</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '2vw' }}>
            ...
            <FormRow label="Machine" labelWidth="5vw" ml="0vw">
              <CustomSelect
                options={machines}
                value={machine}
                onChange={setMachine}
          ...
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '2vw' }}>
            ...
            <FormRow label="Division" labelWidth="5vw" ml="0vw">
          ...
          <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '2vw' }}>
            ...
            <FormRow label="Component" ml="10vw">
              <CustomSelect
