// src/pages/Defect_Management_Lean_Line_ENT_POPUP.tsx 

import '../MyApp.css'
import './Defect_Management_Lean_Line_ENT_POPUP.css'
...
export interface VSM_ENT_PopupProps {
  isOpen: boolean
  onClose: () => void
...
export default function Defect_Management_Lean_Line_ENT_POPUP({
  isOpen,
  onClose,
  ...
  const onSubmit = async () => {
    ...
    console.log('[Defect_VSM_EXT_POPUP] Saved payload:', payload)

    // 최소 검증
    const defectQty = Number(qty)
    if (!plant || !workCenter || !line || !material) {
      console.error('[Defect_VSM_EXT_POPUP] plant/workCenter/line/material is required')
      return
    }
    // qty 필수
    if (!qty || Number.isNaN(defectQty)) {
      console.error('[Defect_VSM_EXT_POPUP] qty must be a valid number')
      return
    }
    // defectiveType 필수
    if (!defectiveType) {
      console.error('[Defect_VSM_EXT_POPUP] defectiveType is required')
      return
    }
    // reason 필수
    if (!reason) {
      console.error('[Defect_VSM_EXT_POPUP] reason is required')
      return
    }
    // division 필수
    if (!side) {
      console.error('[Defect_VSM_EXT_POPUP] division(side) is required')
      return
    }
    //inspectionType 필수
    ...
    try {
      ...
      if ('code' in result && result.code === 'DEFECT_QTY_EXCEEDED') {
        ...
        console.warn('[Defect_VSM_EXT_POPUP] Defect qty exceeded')
        ...
        // 200 정상 성공 케이스
        console.log('[Defect_VSM_EXT_POPUP] Distribute defects result:', result)
        // TODO: 성공 메시지/리셋 처리
      }
      ...
      console.error('[Defect_VSM_EXT_POPUP] Distribute defects error:', msg)
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
          ...
          >
            <span>Lean Line External Defect</span>
