// src/pages/Defect_Management_VJ2_VJ3_INT_POPUP 

import '../MyApp.css'
import './Defect_Management_VJ2_VJ3_INT_POPUP.css'
...
export interface ETC_INT_PopupProps {
  isOpen: boolean
  onClose: () => void
...
export default function Defect_Management_Lean_Line_INT_POPUP({
  isOpen,
  onClose,
  ...
  }, [plant, workCenter, line, material, scrapReturn, isOpen])
  ...
  const onSubmit = async () => {
    ...
    console.log('[Defect_ETC_INT_POPUP] Saved payload:', payload)

    // 최소 검증
    const defectQty = Number(qty)
    if (!plant || !workCenter || !line || !material) {
      console.error('[Defect_ETC_INT_POPUP] plant/workCenter/line/material is required')
      return
    }
    // qty 필수
    if (!qty || Number.isNaN(defectQty)) {
      console.error('[Defect_ETC_INT_POPUP] qty must be a valid number')
      return
    }
    // Return일 때 component 필수
    if (scrapReturn === 'R' && !componentVal) {
      console.error('[Defect_ETC_INT_POPUP] component is required when Return is selected')
      return
    }
    // defectiveType 필수
    if (!defectiveType) {
      console.error('[Defect_ETC_INT_POPUP] defectiveType is required')
      return
    }
    // reason 필수
    if (!reason) {
      console.error('[Defect_ETC_INT_POPUP] reason is required')
      return
    }
    // division 필수
    if (!side) {
      console.error('[Defect_ETC_INT_POPUP] division(side) is required')
      return
    }
    //inspectionType 필수
    ...
    try {
      ...
      if ('code' in result && result.code === 'DEFECT_QTY_EXCEEDED') {
        ...
        console.log('[Defect_ETC_INT_POPUP] Defect qty exceeded')
        ...
        // 200 정상 성공 케이스
        console.log('[Defect_ETC_INT_POPUP] Distribute defects result:', result)
        // TODO: 성공 메시지/리셋 처리
      }
      ...
      console.error('[Defect_ETC_INT_POPUP] Distribute defects error:', msg)
    ...
    >
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1px' }}>
          ...
          >
            <span>VJ2 & VJ3 Internal Defect</span>
