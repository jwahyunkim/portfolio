// src/renderer/pages/FTT_Main_Rework.tsx
...
export default function FTT_Main_Rework() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [plantsRes, linesRes, defectsRes, stylesRes, fttStatusRes] =
          await Promise.all([
            oracleApi.getPlants(),
            oracleApi.getLines(),
  ...
  const onClickOK = async () => {
    ...
    try {
      ...
      await oracleApi.saveFttResultV1({
        material_cd: selectedStyle, // 테스트 고정
        style_cd: selectedStyle,
        size_cd: "", // 이 화면은 size 없음 (백엔드에서 허용하도록 수정함)
        ftt_type: "R", // 고정
        defect_type,
  ...
  const productionValue =
    fttStatus?.PROD_QTY === null || fttStatus?.PROD_QTY === undefined
      ? "-"
      : String(fttStatus.PROD_QTY);

  const reworkValue =
    fttStatus?.REWORK_QTY === null || fttStatus?.REWORK_QTY === undefined
      ? "-"
      : String(fttStatus.REWORK_QTY);

  const bcValue =
    fttStatus?.BC_QTY === null || fttStatus?.BC_QTY === undefined
      ? "-"
      : String(fttStatus.BC_QTY);

  const fttValue =
    fttStatus?.FTT_RATE === null || fttStatus?.FTT_RATE === undefined
      ? "-"
      : `${fttStatus.FTT_RATE}%`;
      ...
      <div style={footerStyle}>
        <div style={footerGridStyle}>
          <button type="button" style={footerBtnBaseStyle} onClick={onClickBCGrade}>
            <div style={footerBtnTextStyle}>B/C Grade</div>
          </button>
          ...
          <button type="button" style={footerBtnBaseStyle} onClick={onClickRework}>
